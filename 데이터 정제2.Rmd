...
summary(reg_for)
reg_for2 <- step(reg_for, scope=list(lower= reg_for,upper = reg1), direction = 'forward')
# 전진 선택에서 사설학원수 독립변수 제거한 lm  
reg_for3 <- lm(부캐관련검색량~ 인구.십만명당.자살률.a. + X1인가구.독거노인.제외..비율.a.+ `여가활용 만족도`+ `삶의 만족도`, data= rdf)
vif(reg_for3)
...
```{r}
shapiro.test(rdf$부캐관련검색량) #정규성검정
#잔차의 정규성 검정
